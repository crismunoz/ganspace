...
def make_row_images(latent, lat_mean, lat_comp, lat_stdev, act_mean, act_comp, act_stdev, scale=1, row_idx=0, n_cols=11, make_plots=True, edit_type='latent'):
    from notebooks.notebook_utils import create_strip_centered
...
if __name__ == '__main__':
    ...
    for i,(val,tval) in enumerate(zip(values[args.start:],tag_values[args.start:])):
        os.makedirs(os.path.join(images_dir,f'images_{tval}'), exist_ok=True)
        img_id = 0
        for _ in tqdm(range(totam_n_imgs)):
            latents = model.sample_latent(n_samples=10)                    
            for ii in range(10):
                img_id+=1
                z = latents[ii][None, ...]
                # Summary grid, real components
                for edit_mode in edit_modes:
                    images = make_row_images(z, tensors.Z_global_mean, tensors.Z_comp, tensors.Z_stdev, tensors.X_global_mean, 
                                tensors.X_comp, tensors.X_stdev, scale=args.sigma, edit_type=edit_mode, row_idx=CX, n_cols=len(values))
                    
                    img_path = os.path.join(images_dir, f'images_{tval}', f"img_{img_id}.jpg")
                    img = (images[i] * 255).astype(np.uint8)[:,:,::-1]
